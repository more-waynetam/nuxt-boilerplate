...
const handleSave = async () => {
  ...
  if (agent._id) {
    await chatStore.updateAgent(agent);
    emits('updateSuccess')
  } else {
    await chatStore.addAgent(agent);
    emits('addSucccess')
  }
  isSaving.value = false;
};

const initForm = (agent?: Agent) => {
  if (!agent) {
    agent = {
      name: "defaultName",
      description: "",
  ...
  agentId.value = agent._id;
  nameField.value = agent.name;
  descField.value = agent.description;
  introField.value = agent.instructions;
  modelField.value = agent.model;
  toolsField.value = agent.tools.map((tool: any) => tool.type);
};
